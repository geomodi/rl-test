...
// Airtable configuration - Updated to use fresh CSV uploads
const AIRTABLE_CONFIG = {
    baseId: 'app7ffftdM6e3yekG',
    tables: {
        ghl: 'tblcdFVUC3zJrbmNf',           // Fresh GHL table
        pos: 'tblHyyZHUsTdEb3BL',           // Fresh POS table
        googleAds: 'tblRBXdh6L6zm9CZn'      // Fresh Google Ads table
    }
};
...
async function loadAirtableData(forceRefresh = false) {
    ...
    try {
        ...
        // Load POS data
        const loadPosData = airtableService.getPOSData({ forceRefresh })
            .then(data => {
        ...
        // Load Google Ads data with cache busting to ensure we get all 1970 records
        const loadGoogleAdsData = airtableService.getGoogleAdsData({
    ...
    async fetchMetaAdsSummaryData() {
        ...
        try {
            ...
            const response = await fetch('/api/airtable/records?' + new URLSearchParams({
                baseId: 'app7ffftdM6e3yekG',
                tableId: 'tblIQXVSwtwq1P4W7' // Meta Ads Summary table
            }));
...
class MetaAdsAnalyticsService {
    ...
    async loadPerformanceCategories() {
        try {
            ...
            const params = new URLSearchParams({
                baseId: 'app7ffftdM6e3yekG',
                tableId: 'tblm9kp1FStqAywPN' // Meta Ads Performance table ID
            });
